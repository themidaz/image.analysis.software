...
private: System::Windows::Forms::Button^  crop_button;
	...
	//initializing and definining the properties  of GUI elements, such as size,appearance ,location on screen etc
...
#pragma endregion
		/*end initialization and defining of GUI*/

		//event listeners are added to some GUI elements like buttons so that we are notified when an event occurs (eg user presses a button) then
		//perform a certain action when that event is triggered

	/*a tabcontrol item was used to achieve a tabbed layout in the software. this seperates it into different sections. 
	The DrawItem event Occurs when the TabControl needs to paint each of its tabs 	
	*/

	private: System::Void tabControl1_DrawItem(System::Object^  sender, System::Windows::Forms::DrawItemEventArgs^  e) {

				make_tabcontol_display_tabs_vertically_and_text_horizontally(sender, e);// call this function and pass on the sender of the event and the object containing the data about the event
	}

		//by default the tabcontrol item can only display tabs horizontally
		//this method (function) will customise the tabcontrol item to make it display  tabs vertically with text displayed horizontally
	...
	{
			System::Drawing::Graphics^ g = e->Graphics; //create the Graphics object. It enables us  to draw lines and shapes, render text, or display and manipulate images.
			System::Drawing::Brush^ _textBrush; //bush is an object used to fill the interiors of graphical shapes such as rectangles, ellipses, pies etc

			// Get a tab from the tabcontrol item.
			TabPage^ _tabPage = tabControl1->TabPages[e->Index];
			
			// Get the boundaries for the rectangle.

			System::Drawing::Rectangle _tabBounds = tabControl1->GetTabRect(e->Index); //returns A Rectangle that represents the bounds of the specified tab in the tabcontrol item.
...

			{
				...
				lasttabrect.X + lasttabrect.Width + tabControl1->Left,	//coordinates of the empty spaces around the tab
					tabControl1->Top + lasttabrect.Y,
					tabControl1->Width - (lasttabrect.X + lasttabrect.Width),
					lasttabrect.Height);

				System::Drawing::Brush^ b = System::Drawing::Brushes::Black; // the color you want	g->FillRectangle(b, emptyspacerect) to be filled in;
				
				// Draw a different background color when a tab is in focus (is selected) , to make it different from the others.
				_textBrush = gcnew System::Drawing::SolidBrush(System::Drawing::Color::Blue);// color of the title of the tab
				g->FillRectangle(System::Drawing::Brushes::Gray, e->Bounds);// paint the bounds of the selected tab in gray
			...
			// Use a different font from the default one
	...
	/*//our app is borderless, so by default draggging it to other parts of the screen is disabled. added some code in the MouseDown()
	...
	private: System::Void exitbutton_Click(System::Object^  sender, System::EventArgs^  e){
		Application::Exit();
	 	...
	 	OpenFileDialog^ dialogOpen = gcnew OpenFileDialog();//file dialog which helps the user to load an image into the software by browsing for an image on their computer
		//the object is placed outisde of any function so that it can be  accessible from different parts of the program
		
	//event triggered when user presses "open" button to browse for an image on their computer		
private: System::Void button1_MouseClick(System::Object^  sender, System::Windows::Forms::MouseEventArgs^  e) {

	dialogOpen->Filter = "Image(*.bmp; *.jpg; *.png; *.gif)|*.bmp;*.jpg;*.png;*.gif|All files (*.*)|*.*||"; //filter ensuring that only images will be accepted into the program
	...
	Bitmap^ bmpSrce = gcnew Bitmap(dialogOpen->FileName); //convert the selected image into a  bitmap object and
	pictureBox1->Image = bmpSrce;//display it in the pictureboxes
	pictureBox3->Image = bmpSrce;
	pictureBox7->Image = bmpSrce;
	pictureBox5->Image = bmpSrce;
	/*pictureboxes in Winforms can only  display images in bitmap format hence the need for conversion*/	
}	

	//event triggered when the value of the trackbar for changing image brightness is changed
private: System::Void brightness_level_tbar_ValueChanged(System::Object^  sender, System::EventArgs^  e) {
	...
	int brightness_threshold = userInterface::brightness_level_tbar->Value; //set the value of variable brightness_threshhold to the value of the trackbar
	Mat image, result; //create two Mat objects
	mw_kwithu_lib::collection_of_improcessing_functions funct_coll; //this object helps us to call functions that are in "mw_kwithu_lib"
	image = imread(funct_coll.ConvertString2Char(dialogOpen->FileName)); // use imread function to read image chosen by user
	result = funct_coll.change_brightness(brightness_threshold, image);//after processing is done, the result is returned and assigned to the mat "result"
	pictureBox6->Image = funct_coll.Mat2Bitmap(result);//convert the mat to a bitmap and display it in the picturebox
	pictureBox6->Refresh();//refresh picturebox
}

		 ////event triggered when the value of the trackbar for changing image contrast is changed
private: System::Void contrast_level_tbar_ValueChanged(System::Object^  sender, System::EventArgs^  e) {

	pictureBox8->Controls->Clear();//first Empty the picture box's contents
	int contrast_threshold = userInterface::contrast_level_tbar->Value;//set the value of variable contrast_threshold to the value of the trackbar
	Mat image, result;//create two Mat objects
	mw_kwithu_lib::collection_of_improcessing_functions funct_coll;//this object helps us to call functions that are in "mw_kwithu_lib"
	image = imread(funct_coll.ConvertString2Char(dialogOpen->FileName)); // use imread function to read image chosen by user
	result = funct_coll.change_contrast(contrast_threshold, image);//after processing is done, the result is returned and assigned to the mat "result"
	pictureBox8->Image = funct_coll.Mat2Bitmap(result);//convert the mat to a bitmap and display it in the picturebox
	pictureBox8->Refresh();
}
...
private: System::Void button9_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	image = imread(funct_coll.ConvertString2Char(dialogOpen->FileName)); 
	...
	//removing gaussian noise from color images	
private: System::Void NonLocalMeansDenoising_Color_ValueChanged(System::Object^  sender, System::EventArgs^  e) {
	...
	pictureBox4->Refresh();
		 ...
		 //removing gaussian noise from greyscale images	
...
private: System::Void periodicNoiseRemoval_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	Mat image, result;
	mw_kwithu_lib::collection_of_improcessing_functions funct_coll;
	image = imread(funct_coll.ConvertString2Char(dialogOpen->FileName), cv::IMREAD_GRAYSCALE); // resize image
...
private: System::Void button20_Click(System::Object^  sender, System::EventArgs^  e) {
	pictureBox8->Controls->Clear();
	Mat image, result;
	mw_kwithu_lib::collection_of_improcessing_functions funct_coll;
	image = imread(funct_coll.ConvertString2Char(dialogOpen->FileName)); 
	// image = funct_coll.ToResize(imread(funct_coll.ConvertString2Char(dialogOpen->FileName)))
...
private: System::Void Negative_effect_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	pictureBox8->Refresh();
		...
		//sharpening an image using unsharp masking.unsharp masking makes an image sharper by adding an edge image (obtained by subtracting a gaussian blurred copy of the image from the original) to the original image.
		// two trackbars are used,sharpenImg_ksize_tBar determines the size of kernel used for the gaussian blur; and sharpenImg_sigma_tBar represents the standard deviation
...
private: System::Void sharpenImg_sigma_tBar_ValueChanged(System::Object^  sender, System::EventArgs^  e) {
	...
	//adding a gaussian blur to an image
 private: System::Void blur_tBar_ValueChanged_1(System::Object^  sender, System::EventArgs^  e) {
	...
	//applying cartoon effect on an image
...
private: System::Void button18_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	pictureBox8->Refresh();
		 ...
		 //sharpening an image's details using opencv's in built function. results are similar to carrying out unsharp masking
...
private: System::Void button22_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	//applying stylization effect on an image
...
private: System::Void mirrror_horizontally_button_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	pictureBox8->Refresh();
		 ...
		 //rotate an image by 90 degrees anticlockwise
...
private: System::Void rotate90_right_button_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	pictureBox8->Refresh();
		 ...
		 Boolean mouseClicked;// true if the mouse has been clicked on the image, false otherwwise
		 System::Drawing::Point startPoint;// x&y coordinates of the point on the image where the mouse is licked
		 System::Drawing::Point endPoint; // x&y coordinates of the point on the image where the mouse is licked
