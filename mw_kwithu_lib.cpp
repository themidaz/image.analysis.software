...
System::Drawing::Bitmap^ collection_of_improcessing_functions::Mat2Bitmap(cv::Mat & colorImage)
{
 ...
 cv::Mat collection_of_improcessing_functions::ToResize(cv::Mat src) {	// Rezise image method
	cv::Mat dst;
	cv::resize(src, dst, cv::Size(400, 400), 0, 0, 1); // we need to define new size onn cv:Sizze(width, height)
	return dst;
}

 //changes back&fore ground colors of the of the button (from the image filters section of the app) which has been pressed, in order to highlight it
  void collection_of_improcessing_functions::highlight_Button(System::Windows::Forms::Control^ control, System::EventArgs^  e)
 {
	  control->ForeColor = System::Drawing::Color::Black;
	  control->BackColor = System::Drawing::Color::Transparent;
 }
  //revert the buttons color to the app's default after the mouse leaves the button
  void collection_of_improcessing_functions::revert_buttonColors(System::Windows::Forms::Control^ control, System::EventArgs^  e)
  {
...
{
	cv::Mat dest, dest1, dest2;
	cvtColor(inputMat, dest, CV_BGR2GRAY);
	GaussianBlur(dest, dest, cv::Size(3,3), 2); 
...
{
	hsv_saturation_processing(saturation_value, inputMat);
	return inputMat;
}
...
cv::Mat collection_of_improcessing_functions::apply_diffuse_effect(cv::Mat inputMat)
{
	cv::Mat dest;
	GaussianBlur(inputMat, dest, cv::Size(7, 7), 1.5, 1.5);
	return dest;
}
cv::Mat collection_of_improcessing_functions::apply_gaussian_blur(cv::Mat inputMat)
{
	cv::Mat dest;
	GaussianBlur(inputMat, dest, cv::Size(7, 7), 1.5, 1.5);
	return dest;
...
{
	if (kernel_size % 2 == 0) //changing kermnel_size to odd if it is even
		kernel_size += 1;
	cv::Mat dest;
	cv::medianBlur(inputMat, dest, kernel_size);
	return dest;
}
/*functions for reducing periodic noise using notch reject filter*/
void collection_of_improcessing_functions::fftshift_(const cv::Mat& inputImg, cv::Mat& outputImg)
{
...
void collection_of_improcessing_functions::filter2DFreq_(const cv::Mat& inputImg, cv::Mat& outputImg, const cv::Mat& H)
{
	...
	cv::Mat abs0 = abs(planes[0]);
	cv::Mat abs1 = abs(planes[1]);
...
void collection_of_improcessing_functions::synthesizeFilterH(cv::Mat& inputOutput_H, cv::Rect roi)
{
	cv::Rect roiA2 = roi, roiA3 = roi, roiA4 = roi;
...
void collection_of_improcessing_functions::synthesizeFilterH_(cv::Mat& inputOutput_H, cv::Point center, int radius)
{
...
void collection_of_improcessing_functions::calcPSD(const cv::Mat& inputImg, cv::Mat& outputImg, int flag)
{
	cv::Mat planes[2] = { cv::Mat_<float>(inputImg.clone()), cv::Mat::zeros(inputImg.size(), CV_32F) };
	cv::Mat complexI;
	merge(planes, 2, complexI);
	...
	{
		cv::Mat imglogPSD;
		imglogPSD = imgPSD + cv::Scalar::all(1);		//switch to logarithmic scale
...
cv::Mat collection_of_improcessing_functions::reduce_periodic_noise(cv::Mat& inputMat)
{
	...
	//imwrite("periodic noise reduction", imgOut);
	return imgOut;
    /*imgIn.convertTo(imgIn, CV_8U);
	normalize(imgIn, imgIn, 0, 255, cv::NORM_MINMAX);
	...
	fftshift(H, H);
	normalize(H, H, 0, 255, cv::NORM_MINMAX);
	return H;
...
{
	cv::Mat NoiseArray = source.clone();
	cv::RNG rand_num_gen;
...
cv::Mat collection_of_improcessing_functions::Add_salt_and_pepper_Noise(cv::Mat &srcArr, float pa, float pb)
{
	cv::RNG rng; // rand number generate，实现随机数产生的类
...
cv::Mat collection_of_improcessing_functions::convert_rgbimg2binaryimg(cv::Mat inputMat)
{
	...
	cv::threshold(greyMat , greyMat, 100, 255, CV_THRESH_BINARY);
	return greyMat;
}
cv::Mat collection_of_improcessing_functions::prewittEdgeDetector(cv::Mat & image, const unsigned char threshold)
{
	std::vector<int> xGradient = { -1, 0, 1, -1, 0, 1, -1, 0, 1 };

	std::vector<int> yGradient = { -1, -1, -1, 0, 0, 0, 1, 1, 1 };
...
void collection_of_improcessing_functions::edgeDetection(cv::Mat & image, std::vector<int> & xGradient, std::vector<int> & yGradient)
{
	...
	if (image.channels() != 1)
	{
		cvtColor(image, tempImage, cv::COLOR_BGR2GRAY);
...
cv::Mat collection_of_improcessing_functions::apply_cartoon_effect(const cv::Mat inputMat)
{
		...
		cv::adaptiveThreshold(gray, edges, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C , cv::THRESH_BINARY, 9, 3); //cv::ADAPTIVE_THRESH_MEAN_C
...
//code for histogram eq gave an error whenever i ran it, didnt have enough time to debug it
cv::Mat collection_of_improcessing_functions::histogram_equalization(cv::Mat inputMat)
{
	/*// Convert BGR image to YCrCb
	cv::Mat ycrcb,result;
	cvtColor(inputMat, ycrcb, cv::COLOR_BGR2HSV);

	// Extarer each of the 
	std::vector  < cv::Mat > channels;
	cv::split(ycrcb, channels);

	// Equalize Y-channel histogram (brightness) 
...
cv::Mat collection_of_improcessing_functions::apply_pixelate_effect(cv::Mat inputMat)
{
	...
	for (int r = 0; r < inputMat.rows; r += pixel_block_size)
	{
		for (int c = 0; c < inputMat.cols; c += pixel_block_size)
		{
			...
			// verify if the pixel is within the region of interessi se encuentra dentro del ROI
...
cv::Mat collection_of_improcessing_functions::color_temp_mild(cv::Mat inputMat)
{
	return inputMat;
}
...
{
	/*the filter enhances the details in and image, thereby making the image look sharper.*/
	cv::Mat dest;
